...
#define AI_NET_OBJ_INSTANCE g_commands_model
 
#undef AI_COMMANDS_MODEL_MODEL_SIGNATURE
#define AI_COMMANDS_MODEL_MODEL_SIGNATURE     "0x368954587cbad0406580f73ad7839ffd"
...
#define AI_TOOLS_DATE_TIME   "2026-02-16T20:45:42+0100"

#undef AI_TOOLS_COMPILE_TIME
#define AI_TOOLS_COMPILE_TIME    __DATE__ " " __TIME__

#undef AI_COMMANDS_MODEL_N_BATCHES
#define AI_COMMANDS_MODEL_N_BATCHES         (1)

static ai_ptr g_commands_model_activations_map[1] = AI_C_ARRAY_INIT;
static ai_ptr g_commands_model_weights_map[1] = AI_C_ARRAY_INIT;
...
AI_TENSOR_OBJ_DECLARE(
  ...
  2, 0x0,
  AI_SHAPE_INIT(4, 33, 16, 1, 1), AI_STRIDE_INIT(4, 4, 132, 2112, 2112),
...
AI_NETWORK_OBJ_DECLARE(
  ...
  AI_BUFFER_INIT(AI_FLAG_NONE,  AI_BUFFER_FORMAT_U8,
    AI_BUFFER_SHAPE_INIT(AI_SHAPE_BCWH, 4, 1, 2380, 1, 1),
    2380, NULL, NULL),
  AI_BUFFER_INIT(AI_FLAG_NONE,  AI_BUFFER_FORMAT_U8,
    AI_BUFFER_SHAPE_INIT(AI_SHAPE_BCWH, 4, 1, 196, 1, 1),
    196, NULL, NULL),
  AI_TENSOR_LIST_IO_OBJ_INIT(AI_FLAG_NONE, AI_COMMANDS_MODEL_IN_NUM, &serving_default_keras_tensor_150_output),
  AI_TENSOR_LIST_IO_OBJ_INIT(AI_FLAG_NONE, AI_COMMANDS_MODEL_OUT_NUM, &nl_2_output),
  &gemm_0_layer, 0x6916d3f4, NULL)
  ...
  ai_handle network, ai_network_report* report)
{
  ...
  {
    ai_network_report r = {
      .model_name        = AI_COMMANDS_MODEL_MODEL_NAME,
      .model_signature   = AI_COMMANDS_MODEL_MODEL_SIGNATURE,
  ...
  ai_handle* network, const ai_handle activations[], const ai_handle weights[])
{
  ...
  err = ai_commands_model_create(network, AI_COMMANDS_MODEL_DATA_CONFIG);
  if (err.type != AI_ERROR_NONE) {
    return err;
  }
  
  if (ai_commands_model_data_params_get(&params) != true) {
    err = ai_commands_model_get_error(*network);
    return err;
  }
#if defined(AI_COMMANDS_MODEL_DATA_ACTIVATIONS_COUNT)
  /* set the addresses of the activations buffers */
  for (ai_u16 idx=0; activations && idx<params.map_activations.size; idx++) {
    AI_BUFFER_ARRAY_ITEM_SET_ADDRESS(&params.map_activations, idx, activations[idx]);
  }
#endif
#if defined(AI_COMMANDS_MODEL_DATA_WEIGHTS_COUNT)
  /* set the addresses of the weight buffers */
  for (ai_u16 idx=0; weights && idx<params.map_weights.size; idx++) {
    AI_BUFFER_ARRAY_ITEM_SET_ADDRESS(&params.map_weights, idx, weights[idx]);
  }
#endif
  if (ai_commands_model_init(*network, &params) != true) {
    err = ai_commands_model_get_error(*network);
  }
  return err;
}
...
AI_API_ENTRY
ai_i32 ai_commands_model_forward(ai_handle network, const ai_buffer* input)
{
